...
public class helperClass {
	...
	public static List<Map<String, Object>> Not_Select(String sql){
		...
		String URL="jdbc:sqlserver://localhost:你的端口; DatabaseName=你的数据库名称";
...
//        crs.populate(rs);
	...
	public static List<Map<String, Object>> SelectSQL(String Bsql) {
		...
		//连接数据库URL
		//String URL ="jdbc:odbc:Driver=Microsoft Access Driver (*.mdb);DBQ=d:\\LJQ\\SHOP.mdb";
		String URL="jdbc:sqlserver://localhost:端口号; DatabaseName=数据库名称";
		Connection conn = null;
		Statement stmt = null;
		ResultSet rs=null;
		try {
...
//        crs.populate(rs);
	...
	public static boolean Not_ZSG(String sql) {
		...
		//连接数据库URL
		//String URL ="jdbc:odbc:Driver=Microsoft Access Driver (*.mdb);DBQ=d:\\LJQ\\SHOP.mdb";
		String URL="jdbc:sqlserver://localhost:端口; DatabaseName=数据库名";
//		Connection conn;
		try {
			conn = DriverManager.getConnection( URL,"用户名", "密码");
		} catch (SQLException e) {
